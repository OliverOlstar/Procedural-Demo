...
namespace Scriptionary
{
		...
		where TItem : IScriptionaryItem
	{
		...
		public TItem GetValue()
		{
			...
			{
				ODev.DebugUtil.DevException($"{GetType().Name}.GetValue() Scriptionary reference is null");
				return default;
			}
			if (!m_Scriptionary.TryGet(m_Key, out TItem value))
			{
				ODev.DebugUtil.DevException($"{GetType().Name}.GetValue() Key '{m_Key}' in {m_Scriptionary.name}");
				return default;
			}
			return value;
		}
