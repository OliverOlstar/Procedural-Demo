...
public class AssetPickerPathSource : IAssetPickerPathSource
{
	...
	{
		List<System.Type> actualTypes = ODev.ListPool<System.Type>.Request();
		string[] typesNames = new string[types.Length];
		for (int i = 0; i < types.Length; i++)
		{
			System.Type type = ODev.PropertyDrawerUtil.GetUnderlyingType(types[i]);
		...
		m_Title = ODev.Str.BuildWithBetweens(" & ", typesNames);
		m_TypesToLoad = actualTypes.ToArray();
		m_CanBeNested = canBeNested;
		m_PathPrefix = limitPathsByPrefix;

		ODev.ListPool<System.Type>.Return(actualTypes);
	}
	...
	List<string> IAssetPickerPathSource.GetPaths()
	{
		...
		{
			paths.AddRange(ODev.AssetDatabaseUtil.Find(type, m_CanBeNested));
		}
