...
public static class AssetPickerUtilityMenu
{
		...
		System.Action<SerializedProperty, string> onSelected = null)
	{
			...
			r.Contains(e.mousePosition))
		{
			...
			foreach (System.Type type in ODev.TypeUtility.GetMatchingTypes(objectType, IsTypeMatching))
			{
				Context context = new()
				{
					Type = type,
	...
	/// <summary>Accessing ODev.SOCacheName.name causes exceptions, utility function to avoid that</summary>
	public static string GetObjectName(Object obj) => obj is ODev.SOCacheName cachedName ? cachedName.Name : obj.name;
		...
		System.Type objectTypeHint = null)
	{
		...
		else
		{
			...
			string[] paths = ODev.AssetDatabaseUtil.Find(objectTypeHint);
			if (paths.Length != 0)
			{
				examplePath = paths[0];
			}
