...
namespace ODev.Picker
{
	public class AssetPickerRenameWindow : EditorWindow
	{
		public static void Open(Object obj, System.Type assetType, Vector2 position)
		{
			...
			string[] paths = ODev.Util.AssetDatabase.Find(assetType);
			List<Object> assets = new();
			ODev.Util.AssetDatabase.LoadAll(assetType, assets);
		...
		private readonly List<GUIContent> m_OtherAssets = new();
		private readonly List<(string, string, Texture)> m_OtherAssets2 = new();
		...
		private void OnGUI()
		{
			...
			itemRect = UberPickerGUI.IndentRect(itemRect);
