...
namespace ODev.Data.Validate.String
{
	public class AssetExistsAttribute : StringAttribute
	{
		private readonly System.Type[] m_Types = default;
		private readonly string[] m_Paths = null;
		...
		protected override string ValidateStringValue(string value)
		{
			...
			{
				bool exists = AssetDatabase.Exists(value, m_Types);
				return exists ? null : $"Could not find asset of type {m_Types}";
			}

			foreach (string path in m_Paths)
			{
				string[] assetPaths = AssetDatabase.FindAll(value, m_Types);
	...
	{
		private readonly System.Type m_Type = default;
		...
		public PrefabExistsWithComponentAttribute(System.Type componentType, bool allowBlanks = false, string path = null, string columnNameToDisplayInErrors = null) : 
			base(allowBlanks, columnNameToDisplayInErrors)
		{
			if (!componentType.IsInterface && !componentType.Is(typeof(Component)))
			{
		...
		protected override string ValidateStringValue(string value)
		{
			...
			{
				GameObject prefab = AssetDatabase.Load<GameObject>(value);
				if (!prefab)
				{
			...
			{
				string path = AssetDatabase.Find(value, typeof(GameObject));
	...
	public class ForeignKeyAttribute : StringAttribute
	{
		...
		protected override string OnValidateAttribute()
		{
			...
			foreach (System.Type type in m_Type)
			{
				...
				{
					return Str.Build("Data base type ", type.Name, " does not exist");
				}
			}
			return null;
		}
		...
		protected override string ValidateStringValue(string value)
		{
			...
			Str.Add("Could not find foreign key in ", m_Type[0].Name);
			for (int i = 1; i < m_Type.Length; i++)
			{
				Str.Add(" or ", m_Type[i].Name);
			}
			return Str.Finish();
		}
	}
	...
	public class ExistsInCatalogAttribute : StringAttribute
	{
		...
		protected override string OnValidateAttribute()
		{
			...
			{
				return Str.Build("Catalog of type '", m_CatalogType.Name, "' does not exist");
			}
			return null;
		}
