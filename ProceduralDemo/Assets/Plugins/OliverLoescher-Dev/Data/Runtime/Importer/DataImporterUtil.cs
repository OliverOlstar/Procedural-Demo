...
namespace ODev.Data
{
	...
	{
		public static readonly DebugOption IncludeVariants = new DebugOption.StringWithDropdown(DebugOption.Group.Data, "Include Variants", GetVariants(), DebugOption.DefaultSetting.OnDevice, DebugOption.ReleaseSetting.AlwaysOn, 
	...
	public class DataImporterUtil
	{
		...
		public static void AssignField(object entry, Raw.Sheet sheet, Raw.Row row, Raw.ReflectionCache refCache, object defaultEntry = null)
		{
			foreach (Raw.ReflectionCache.ImportField f in refCache.ImportFields)
			{
				...
				if (!row.TryGetValue(f.ColumnName, out object rawValue) || rawValue == null)
				{
					if (defaultEntry != null)
					{
						f.Info.SetValue(entry, f.GetDefaultValue(defaultEntry));
			...
			Raw.ReflectionCache refCache)
		{
			foreach (Raw.ReflectionCache.ImportMethod m in refCache.ImportMethods)
			{
				...
				if (!row.TryGetValue(m.ColumnName, out object rawValue) || rawValue == null)
				{
					continue;
				}
				if (TryParseValue(
