...
namespace ODev.Data
{
	public class DataValidationUtil : LockEditor<DataValidationUtil>
	{
		...
		public static string BuildSummaryReport()
		{
			...
			Str.AddLine("Validation failed with ", s_Count.ToString(), " errors:");
			Str.AddLine();
			foreach (KeyValuePair<string, VariantErrors> variantErrors in s_Errors)
			{
				foreach (KeyValuePair<string, SheetErrors> sheet in variantErrors.Value)
				{
					...
					Str.AddLine(variantErrors.Key.ToString(), "-", string.IsNullOrEmpty(sheet.Key) ? "Misc" : sheet.Key, " errors: ", count.ToString());
				}
			}
			string result = Str.Finish();
		...
		public static string BuildFullReport()
		{
			...
			foreach (KeyValuePair<string, VariantErrors> variantErrors in s_Errors)
			{
				Str.AddLine(variantErrors.Key.ToString());
				foreach (KeyValuePair<string, SheetErrors> sheet in variantErrors.Value)
				{
					Str.AddLine("   " + (string.IsNullOrEmpty(sheet.Key) ? "Misc" : sheet.Key));
					foreach (KeyValuePair<string, RowErrors> row in sheet.Value)
					{
						foreach (KeyValuePair<string, ColumnErrors> columns in row.Value)
						{
		...
		public static void Raise(
			...
			string error)
		{
			string s = Str.Build("[", rowName, ",", columnName, "] = {", value, "} ", error);
		...
		public static void Raise(
			...
			string error)
		{
			string s = Str.Build("[", rowName, ",", columnName, "] ", error);
			Add(s, sheetName, rowName, columnName);
		}
		...
		public static void Raise(
			...
			string error)
		{
			string s = Str.Build("[", rowName, "] ", error);
