		...
		where TData : IDataDictItem
	{
		...
		public static TSingleton Instance
		{
			get
			{
				...
				{
					typeof(TSingleton).LogError("Instance Unavaliable make sure DBManager is initialized before calling Instance");
				}
				return instance;
			}
		}
		...
		{
			List<TDataStruct> data = ListPool<TDataStruct>.Get();
			int count = 0;
			if (m_RawData == null)
			{
				this.DevException($"{GetType().Name}.Initialize() Object {name} serialized null RawData {typeof(TDataStruct).Name}. " +
			...
			OnInitialize();
			ListPool<TDataStruct>.Release(data);
		}
		...
		sealed public override void PostProcess(Raw.Sheet sheet)
		{
			if (m_RawData == null)
			{
				this.DevException($"RawData in {sheet.SheetName} is currently null");
			}
		...
		{
			HashSet<System.Type> dataTypes = HashSetPool<System.Type>.Get();
			foreach (TData data in m_RawData)
			{
			...
			{
				foreach (System.Type t in Util.Types.GetMatchingTypes(_EditorGenericDataBaseType, IsDerivedFrom))
				{
			...
			HashSetPool<System.Type>.Release(dataTypes);
		}
