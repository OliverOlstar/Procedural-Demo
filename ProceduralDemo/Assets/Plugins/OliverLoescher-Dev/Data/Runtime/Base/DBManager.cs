...
namespace ODev.Data
{
	public static class DBManager
	{
		...
		public static void Initialize(string variant, IEnumerable<DBSO> dataSOs)
		{
			...
			List<DBBase> dbs = ListPool<DBBase>.Get();
			List<DataSourceBase> srcs = ListPool<DataSourceBase>.Get();
			foreach (ScriptableObject so in dataSOs)
			{
				if (so == null)
				{
					typeof(DBManager).DevException($"DBManager.Initialize() Null data, a DB ScriptableObject is probably corrupted, ie. missing script reference");
				}
				...
				{
					typeof(DBManager).DevException($"DBManager.Initialize() {so.name} has invalid type {so.GetType()}");
				}
			}
			...
			ListPool<DBBase>.Release(dbs);
			ListPool<DataSourceBase>.Release(srcs);
		}
		...
		private static void AddDB(DBBase db, List<DataSourceBase> srcs)
		{
			...
			{
				typeof(DBManager).LogError("DBManager.AddDB() Cannot load two DBs of the same type " + key);
				return;
			}
			DBBase instance = Object.Instantiate(db);
		...
		private static bool EditorTryInitializeDB(string path, out DBBase dataBase)
		{
			...
			List<DataSourceBase> sources = ListPool<DataSourceBase>.Get();
			string[] sourcePaths = AssetDatabase.Find(typeof(DataSourceBase));
			...
			ListPool<DataSourceBase>.Release(sources);
...
#if UNITY_EDITOR
			string[] paths = AssetDatabase.Find(dbType);
...
#if UNITY_EDITOR
			string[] paths = AssetDatabase.Find(dataType);
			if (paths.Length == 0)
			{
				data = null;
				return false;
