...
namespace ODev.Data
{
			...
			where TData : IDataDictItem, IDataCollectionItem
	{
		...
		public static TCollection GetCollection(string key)
		{
			...
			{
				typeof(TSingleton).DevException("GetCollection() Cannot pass a null or empty Collection ID");
			}
			if (!Instance.m_Collections.TryGetValue(key, out TCollection rt))
			{
				typeof(TSingleton).DevException($"GetCollection() Data {key} does not exist");
			}
			return rt;
		}
