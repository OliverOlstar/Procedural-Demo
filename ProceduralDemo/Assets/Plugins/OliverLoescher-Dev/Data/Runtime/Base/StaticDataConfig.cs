...
public class StaticDataConfig : ScriptableObject
{
	...
	public Dictionary<string, DataVariant> BuildVariantsDictionary(bool includeBase = false)
	{
		...
		foreach (string variantFolder in GetAllVariantPaths())
		{
			...
			{
				this.DevException($"[ImportData] BuildVariantsDictionary() The variant '{variantName}' is trying to be added twice. Variants can not share names. Please ensure no variant folders at the path '{GetRootPath(RootFolder.Source)}' have matching names.");
				continue;
			}
	...
	public string GetDataVariantPath(DataVariant set, in Dictionary<string, DataVariant> variants)
	{
		...
		while (set.Parent != DataVariant.Base.Name)
		{
			...
			{
				this.LogError($"[{nameof(StaticDataConfig)}] GetDataVariantPath() Data variant path {path} could not complete because varient.parent does not exist.");
				return rootPath;
			}
			path = $"{set}/{path}";
		}
