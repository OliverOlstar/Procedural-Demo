...
namespace ODev.Data
{
	public class ImportData
	{
			...
			HashSet<string> dirty)
		{
			...
			foreach (DBHash pendingHash in hashes.Values)
			{
				if (!masterHashes.TryGetValue(pendingHash.TypeName, out DBHash masterHash))
				{
					...
					Str.AddNewLine(pendingHash.TypeName, " - *new*");
					continue;
				}
				for (int i = 0; i < pendingHash.Hash.Length; i++)
				{
					...
					{
						dirty.Add(pendingHash.TypeName);
						Str.AddNewLine(pendingHash.TypeName, " - ", Func.BytesToString(pendingHash.Hash), " != ", Func.BytesToString(masterHash.Hash));
						break;
					}
				}
			}
			DictionaryPool<string, DBHash>.Release(masterHashes);
		}
			...
			string[] ignoreSheetNames)
		{
			...
			HashSet<Type> types = new(Util.Types.GetMatchingTypes(typeof(DBBase), IsDerivedFrom));
			HashSet<Type> sourceTypes = new(Util.Types.GetMatchingTypes(typeof(DataSourceBase), IsDerivedFrom));
			...
			// FIRST PASS IMPORT
			//////////////////////////////////////////////
			Dictionary<Type, DBBase> dbList = new(types.Count);
			...
			List<DataSourceBase> sourceList = new(sourceTypes.Count);
			HashSet<string> dirty = new();
			...
			Log($"First pass, created {dirty.Count} Data Sources:{Str.Finish()}", variant);
			...
			Log($"Found {dirty.Count} dirty DBs and Data Sources{Str.Finish()}", variant);
			...
			if (nonGeneratedDataDirty) // Only need to run validation pass if some non generated data is dirty
			{
				...
				List<Type> dbDependancies = ListPool<Type>.Get();
				foreach (KeyValuePair<Type, DBBase> pair in dbList)
				{
					db = pair.Value;
					s_Current = db;
				...
				ListPool<Type>.Release(dbDependancies);
				DBManager.DeregisterOnEditorAccessAction(OnDBAccess);
				typeof(ImportData).Log($"[DataImport] [{variant}] Validated following {validatedCount} DBs:\n{message}"); // Extra log for extra info
			...
			typeof(ImportData).Log($"[DataImport] [{variant}] Generated following DBs:\n{generateMessage}");
			...
			Dictionary<string, Type> overrideServerJsonDBs = DictionaryPool<string, Type>.Get();
			foreach (DBBase db in dbList.Values)
			{
			...
			for (int i = 0; i < dataConfig.ServerDataSheetNames.Length; i++)
			{
				...
				string json = null;
				if (overrideServerJsonDBs.TryGetValue(serverSheetName, out Type serverDBType))
				{
