		...
		where TComponent : MaskableGraphic
{
	...
	[SerializeField]
	private Channels m_Channels = Channels.RGBA;
	[SerializeField]
		...
		where TPlayerComponent : MaskableGraphic
	{
		...
		protected override void OnUpdateTween(float normalizedTime, float loopingTime)
		{
			...
			for (int i = 0; i < m_Components.Count; i++)
			{
				Color c = Color.Lerp(m_SourceColors[i], color, src);
				SetColor(m_Components[i], c);
			}
		}
		...
		private void SetColor(TPlayerComponent component, Color c)
		{
			switch (Animation.m_Channels)
			{
				case Channels.RGB:
					c.a = component.color.a;
					component.color = c;
					break;
				default:
					component.color = c;
