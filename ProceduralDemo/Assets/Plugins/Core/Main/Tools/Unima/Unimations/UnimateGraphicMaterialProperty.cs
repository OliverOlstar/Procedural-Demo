	...
	public class Player : UnimaTweenPlayer<UnimateGraphicMaterialProperty>
	{
		...
		protected override void OnStartTween()
		{
			for (int i = 0; i < m_Graphics.Count; i++)
			{
				...
				if (graphic != null && graphic.materialForRendering != null) // Apparently this can happen sometimes, Oko had a case where these materials seemed to get unloaded during hotfix
				{
					m_InitialValues[i] = graphic.materialForRendering.GetFloat(m_PropertyID);
				}
			}
		}
		...
		protected override void OnUpdateTween(float normalizedTime, float loopingTime)
		{
			...
			for (int i = 0; i < m_Graphics.Count; i++)
			{
				...
				{
					graphic.materialForRendering.SetFloat(m_PropertyID, value);
				}
			}
		}
		...
		protected override void OnEndTween(bool interrupted)
		{
			if (Animation.m_RestoreInitialValueOnEnd)
			{
				for (int i = 0; i < m_Graphics.Count; i++)
				{
					...
					if (graphic != null && graphic.materialForRendering != null)
					{
						graphic.materialForRendering.SetFloat(m_PropertyID, m_InitialValues[i]);
					}
				}
			}
			else if (interrupted)
