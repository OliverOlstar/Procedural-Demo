...
namespace Core
{
	public static class Util
	{
		...
		public static void ScaleWorldCanvasToViewport(Canvas canvas, Camera camera, bool resizeToMatchAspect, bool resizeHeight)
		{
			...
			if (resizeToMatchAspect)
			{
				if (resizeHeight)
				{
					if (Approximately0(viewportWidth))
					{
				...
				{
					if (Approximately0(viewportHeight))
					{
		...
		public static bool Approximately0(float a)
		{
		...
		public static bool IsVectorZeroXZ(Vector3 v)
		{
			v.y = 0.0f;
			return Vector3.SqrMagnitude(v) < VECTOR_EPSILON_SQR;
		}
		...
		public static float Normalize(ref Vector3 v, bool zeroLengthInputExpected = false)
		{
			float mag = Mathf.Sqrt(v.x * v.x + v.y * v.y + v.z * v.z);
			if (mag < EPSILON)
			{
				if (zeroLengthInputExpected)
				{
					return mag;
				}
				else
				{
					// If zero length vector is unexpected return EPSILON to avoid potential divide by zero problems
					Debug.Assert(zeroLengthInputExpected, "GameUtil.Normalize() Passed zero vector, this is probably bad...");
					return EPSILON;
				}
			}
		...
		public static float Normalize2(ref Vector2 v, bool zeroLengthInputExpected = false)
		{
			...
			if (mag < EPSILON)
			{
				...
				{
					// If zero length vector is unexpected return EPSILON to avoid potential divide by zero problems
					Debug.Assert(zeroLengthInputExpected, "GameUtil.Normalize2() Passed zero vector, this is probably bad...");
					return EPSILON;
				}
			}
			v /= mag; // Normalize vector
		...
		public static float NormalizeXZ(ref Vector3 v, bool zeroLengthInputExpected = false)
		{
			v.y = 0.0f;
			...
			if (mag < EPSILON)
			{
				...
				{
					// If zero length vector is unexpected return EPSILON to avoid potential divide by zero problems
					Debug.Assert(zeroLengthInputExpected, "GameUtil.NormalizeXZ() Passed zero vector, this is probably bad...");
					return EPSILON;
				}
			}
			v /= mag; // Normalize vector
		...
		{
			if (Approximately0(heightPercent))
			{
		...
		{
			if (Approximately0(Vector2.Distance(start, end)))
			{
				return end;
			}
			if (Approximately0(heightPercent))
		...
		public static bool ExternalHomotheticCenter(Circle2D circle0, Circle2D circle1, out Vector2 ehCenter)
		{
			...
			if (Approximately0(radiiDifference))
			{
		...
		public static float TruncateDecimalPlaces(float value, int places)
		{
			...
			float truncated = Mathf.Floor(Mathf.Abs(value) * f) / f;
			truncated *= Mathf.Sign(value);
			return truncated;
		}
		...
		public static float FOVVertToHoriz(float verticalFOV)
		{
			float ratio = (float)Screen.width / Screen.height;
			float hfov = 2.0f * ATan(Tan(0.5f * verticalFOV) * ratio);
			return hfov;
		}

		public static float FOVHorizToVert(float horizontalFOV)
		{
		...
		public static Vector3 ClosestPointOnPlane(Vector3 planePoint, Vector3 planeNormal, Vector3 point)
		{
			...
			float dot = Vector3.Dot(planeNormal, planeToTarget);
			Vector3 pointOnPlane = point - dot * planeNormal;
			return pointOnPlane;
		}

		public static Vector3 RayPlaneIntersection(Vector3 rayOrigin, Vector3 rayDirection, Vector3 planePoint, Vector3 planeNormal)
		{
			float t = RayPlaneDistance(rayOrigin, rayDirection, planePoint, planeNormal);
			return t * rayDirection + rayOrigin;
		}
		public static float RayPlaneDistance(Vector3 rayOrigin, Vector3 rayDirection, Vector3 planePoint, Vector3 planeNormal)
		{
			float dot = Vector3.Dot(planeNormal, rayDirection);
			if (Approximately0(dot))
			{
				return 0.0f; // Parallel
			}
			float t = Vector3.Dot(planePoint - rayOrigin, planeNormal) / dot;
			return t;
		}
			...
			out Vector3 intersection)
		{
			...
			float distance = (-b - Mathf.Sqrt(discriminant)) / (2.0f * a);
	...
	public struct Circle2D
	{
		...
		public Vector2[] Intersection(Ray2D intersectingRay)
		{
			...
			if (Util.Approximately0(delta) || toCenter.magnitude < radius)
			{
