...
namespace Act2
{
	...
	{
		public static void DrawArrow(Vector2 fromPos, float fromTangentOffset, Vector2 toPos, float toTangentOffset, float width)
		{
			float sign = Mathf.Sign(toTangentOffset);
			if (sign < 0.0f)
			{
				toPos.x -= 9;
			}
			...
			AmplifyShaderEditor.GLDraw.DrawBezier(
				fromPos,
				fromPos + fromTangentOffset * Vector2.right,
				toPos,
				toPos + toTangentOffset * Vector2.right,
				Color.white,
				width);

			Vector2 arrowSize = new Vector2(16, 20);
			if (sign < 0.0f)
			{
				toPos.x -= 3;
			}
			...
			Rect arrowRect = new Rect(
				toPos.x,
				toPos.y - 0.5f * arrowSize.y,
				arrowSize.x,
				arrowSize.y);
			//EditorGUI.DrawRect(arrowRect, Color.magenta);
			GUI.Label(arrowRect, sign < 0.0f ? "\u25ba" : "\u25c0");
		}
			...
			float width)
		{
			...
			fromPos.x = fromSide == RectSide.Right ? fromRect.xMax : fromRect.xMin;
			Vector2 toPos = toRect.center;
			toPos.x = toSide == RectSide.Right ? toRect.xMax : toRect.xMin;

			float fromSign = fromSide == RectSide.Right ? 1.0f : -1.0f;
			float toSign = toSide == RectSide.Right ? 1.0f : -1.0f;

			DrawArrow(
				fromPos,
				fromSign * tangentDist,
				toPos,
				toSign * tangentDist,
				width);
		}
...
// Originally stole this code from AmplifyShaderEditor
