...
namespace RootMotion.FinalIK
{
	...
	public class IKSolverFullBodyInspector : IKSolverInspector {
		...
		public static void AddChain(FBIKChain[] chain, int index, Color color, float size) {
			...
			SphereCap(0, GetNodePosition(chain[index].nodes[^1]), Quaternion.identity, size);

			for (int i = 0; i < chain[index].children.Length; i++) {
				Handles.DrawLine(GetNodePosition(chain[index].nodes[^1]), GetNodePosition(chain[chain[index].children[i]].nodes[0]));
			}
		}
