...
namespace RootMotion.FinalIK
{
	...
	public class IKSolverAimInspector: IKSolverInspector {
		...
		public static void AddInspector(SerializedProperty prop, bool editHierarchy) {
			...
			if (!prop.FindPropertyRelative("XY").boolValue)
			{
		...
		public static void AddScene(IKSolverAim solver, Color color, bool modifiable) {
			...
			{
				ConeCap(0, solver.transform.position, Quaternion.LookRotation(solver.transform.rotation * solver.axis), GetHandleSize(solver.transform.position) * 2f);
			}
			...
			if (Application.isPlaying && solver.IKPositionWeight > 0) {
				...
				Handles.DrawLine(solver.bones[^1].transform.position, solver.transform.position);
