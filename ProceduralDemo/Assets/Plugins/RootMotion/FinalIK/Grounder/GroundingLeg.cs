		...
		public class Leg {
			...
			public void Process() {
                ...
                switch (grounding.quality)
                {
                    ...
                    case Quality.Simple:
                        ...
                        RaycastHit sideHit = GetRaycastHit(0.5f * grounding.footRadius * grounding.root.right);

                        if (heelHit.collider != null || toeHit.collider != null || sideHit.collider != null)
						{
							isGrounded = true;
						}
            ...
            private RaycastHit GetCapsuleHit(Vector3 offsetFromHeel)
            {
                ...
                if (grounding.CapsuleCast(capsuleStart, capsuleEnd, grounding.footRadius, -up, out hit, grounding.maxStep * 2, grounding.layers, QueryTriggerInteraction.Ignore))
                {
                    ...
                    {
                        hit.point = origin - 2f * grounding.maxStep * up;
                        hit.normal = up;
                    }
                }
