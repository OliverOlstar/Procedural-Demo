...
public class AssetPickerPathSource : IAssetPickerPathSource
{
	...
	{
		List<System.Type> actualTypes = OCore.ListPool<System.Type>.Request();
		string[] typesNames = new string[types.Length];
		for (int i = 0; i < types.Length; i++)
		{
			System.Type type = OCore.PropertyDrawerUtil.GetUnderlyingType(types[i]);
		...
		m_Title = OCore.Str.BuildWithBetweens(" & ", typesNames);
		m_TypesToLoad = actualTypes.ToArray();
		m_CanBeNested = canBeNested;
		m_PathPrefix = limitPathsByPrefix;

		OCore.ListPool<System.Type>.Return(actualTypes);
	}
	...
	List<string> IAssetPickerPathSource.GetPaths()
	{
		...
		{
			paths.AddRange(OCore.AssetDatabaseUtil.Find(type, m_CanBeNested));
		}
