...
namespace OCore
{
	...
	public partial class DebugOption : IComparable<DebugOption>
	{
		...
		{
			m_Group = group;
			m_Name = Util.Func.IsRelease() ? $"Release_{name}" : name; // Prevent bugs when installing a release build over top of a dev build
			m_ArgKey = m_Name + "Arg";
		...
		public void SetString<T>(T e) where T : Enum
		{
			Set();
			SetArgInternal(Enum.GetName(typeof(T), e));
		}
...
#if UNITY_EDITOR
			string arg = UnityEditor.EditorPrefs.GetString(m_ArgKey, /*def*/ string.Empty);
#else
			string arg = PlayerPrefs.GetString(m_ArgKey, string.Empty);
#endif
		...
		public bool CanShow()
		{
			return m_ShowIfType switch
			{
				ShowIfType.IfInEditor => Application.isEditor,
				ShowIfType.IfNotInEditor => !Application.isEditor,
				ShowIfType.IfIsPlaying => Application.isPlaying,
				ShowIfType.IfNotIsPlaying => !Application.isPlaying,
				ShowIfType.Never => false,
				// ShowIfType.Always
				_ => true,
			};
		}
