...
namespace OliverLoescher
{
    public abstract class MonoBehaviourSingleton<T> : MonoBehaviour where T : MonoBehaviourSingleton<T>, new()
    {
        ...
        protected static void Log(string pMessage, string pMethodName) => Util.Debug.Log(pMessage, pMethodName, LogContext);
		[Conditional("ENABLE_DEBUG_LOGGING")]
        protected static void LogWarning(string pMessage, string pMethodName) => Util.Debug.LogWarning(pMessage, pMethodName, LogContext);
		[Conditional("ENABLE_DEBUG_LOGGING")]
        protected static void LogError(string pMessage, string pMethodName) => Util.Debug.LogError(pMessage, pMethodName, LogContext);
		[Conditional("ENABLE_DEBUG_EXCEPTIONS")]
        protected static void LogExeception(string pMessage, string pMethodName) => Util.Debug.DevException(pMessage, pMethodName, LogContext);
    }
}